...
class SerialPort {
 public:
  ...
  inline bool returnOldFlag() { return receive_data_.old_flag; }
   ...
   * @param depth 深度信息
   */
  void writeData(const int& data_type, const int& is_shooting, const int16_t& yaw,
                 const int16_t& pitch, const Write_Data::node& cord, const int16_t& depth);
   ...
   * @param _depth        深度
   */
  void updataWriteData(const int _data_type, const int _is_shooting, const float _yaw,
                       const float _pitch, const Write_Data::node _cord, const int _depth);
   ...
   */
  Write_Data gainWriteData(const int _data_type, const int _is_shooting, const float _yaw,
                           const float _pitch, const Write_Data::node _cord, const int _depth);
 ...
 private:
   ...
   * @param  depth            深度
   */
  void getDataForCRC(const int& data_type, const int& is_shooting, const int16_t& yaw,
                     const int16_t& pitch, const Write_Data::node& cord, const int16_t& depth);
   ...
   * @param CRC          CRC 校验码
   */
  void getDataForSend(const int& data_type, const int& is_shooting, const int16_t& yaw,
                      const int16_t& pitch, const Write_Data::node& cord, const int16_t& depth,
                      const uint8_t& CRC);
};
