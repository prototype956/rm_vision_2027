...
void SerialPort::writeData(const int& data_type, const int& is_shooting, const int16_t& yaw,
                           const int16_t& pitch, const Write_Data::node& cord,
                           const int16_t& depth) {
  // getDataForCRC(data_type, is_shooting, _yaw, yaw, _pitch, pitch, depth, predict_cord,
  // predict_length, predict_width); uint8_t CRC = checksumCRC(crc_buff_, sizeof(crc_buff_))
  ...
  if (serial_config_.show_serial_information == 1) {
    ...
    fmt::print(" {} {} {} {} {}", static_cast<float>(yaw_reduction_) / 100,
               static_cast<float>(pitch_reduction_) / 100, static_cast<int>(cord_reduction_x),
               static_cast<int>(cord_reduction_y), static_cast<float>(depth_reduction_));
...
void SerialPort::writeData(const Write_Data& _write_data) {
  ...
  writeData(write_data_.data_type, write_data_.is_shooting, write_data_.yaw, write_data_.pitch,
            write_data_.cord, write_data_.depth);
}
...
// this active
void SerialPort::updataWriteData(const int _data_type, const int _is_shooting, const float _yaw,
                                 const float _pitch, const Write_Data::node _cord,
                                 const int _depth) {
...
Write_Data SerialPort::gainWriteData(const int _data_type, const int _is_shooting, const float _yaw,
                                     const float _pitch, const Write_Data::node _cord,
                                     const int _depth) {
...
void SerialPort::getDataForCRC(const int& data_type, const int& is_shooting, const int16_t& yaw,
                               const int16_t& pitch, const Write_Data::node& cord,
                               const int16_t& depth) {
  crc_buff_[0] = 0x53;
...
void SerialPort::getDataForSend(const int& data_type, const int& is_shooting, const int16_t& yaw,
                                const int16_t& pitch, const Write_Data::node& cord,
