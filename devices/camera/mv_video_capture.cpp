...
void VideoCapture::cameraReleasebuff() {
  // After successfully calling CameraGetImageBuffer, CameraReleaseImageBuffer must be called to
  // release the acquired buffer Otherwise, when CameraGetImageBuffer is called again, the program
  // will hang and block until CameraReleaseImageBuffer is called in another thread
  if (iscamera0_open) {
    CameraReleaseImageBuffer(hCamera, pbyBuffer);
  }
}
