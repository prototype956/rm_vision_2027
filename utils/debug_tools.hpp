...
class Tools {
 public:
  ...
  static void recordInit(std::string video_name, cv::VideoWriter& writer, cv::Size frame_size,
                         int fps) {
  ...
  static void drawDiagram(std::string window_name, std::queue<float>& data, SIZE size,
                          float add_data, std::string x_axis_name, std::string y_axis_name,
                          int y_domain, cv::Scalar color = cv::Scalar(255, 0, 0)) {
    int width;
    int height;
    int plotWidth;
    int plotHeight;
 ...
 private:
  inline static void drawAxis(cv::Mat& img, cv::Point origin, int& width, int& height,
                              cv::Scalar color, int thickness, int& y_domin) {
    ...
    for (int i = 0; i <= 10; ++i) {
      cv::putText(img, formatFloatValue(y_domin * 1.0 / 10.0 * i, fixed),
                  cv::Point(origin.x - 30, origin.y - stepSize * i), cv::FONT_HERSHEY_SIMPLEX, 0.3,
                  cv::Scalar(0, 0, 0));
      cv::line(img, cv::Point(origin.x, origin.y - stepSize * i),
               cv::Point(origin.x + 10, origin.y - stepSize * i), color, thickness);
    }
  }

  inline static void drawGraph(cv::Mat& img, std::vector<float>& data, cv::Scalar color,
                               cv::Point& origin, int& plotWidth, int& plotHeight) {
  ...
  inline static void addLabels(cv::Mat& img, cv::Point origin, int width, int height,
                               std::string xLabel, std::string yLabel) {
    cv::putText(img, xLabel, cv::Point(origin.x + width - 20, origin.y + 20),
                cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 0, 0));
    cv::putText(img, yLabel, cv::Point(origin.x + 10, origin.y - height - 10),
                cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 0, 0));
  }

  inline static void addInfo(cv::Mat& img, float min_data, float max_data) {
    cv::putText(img, fmt::format("min:{:.2f}", min_data), cv::Point(img.cols - 80, 30),
                cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 0, 0));
    cv::putText(img, fmt::format("max:{:.2f}", max_data), cv::Point(img.cols - 80, 60),
                cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 0, 0));
  }
};
};  // namespace tools
