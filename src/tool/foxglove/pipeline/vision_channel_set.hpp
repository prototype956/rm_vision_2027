...
enum class VisionTopic {
  ...
  REAL_TRANSFORMS,       ///< 实机采集时刻 world -> gimbal -> camera_optical。
...
struct ChannelIds {
  ...
  std::uint64_t real_transforms{0};       ///< 实机 TF 频道 ID。
  ...
  std::uint64_t referee_state{0};
  std::uint64_t combat_evaluation{0};
...
struct ChannelPublishResult {
  ...
  std::array<ChannelPublishError, 29> errors{};  ///< 每个固定话题最多记录一个错误。
...
class VisionChannelSet final {
 ...
 private:
  ...
  std::unique_ptr<::foxglove::schemas::FrameTransformsChannel> real_transforms_;  ///< 实机 TF。
  ...
  std::unique_ptr<::foxglove::RawChannel> referee_state_;
  std::unique_ptr<::foxglove::RawChannel> combat_evaluation_;
  std::unique_ptr<::foxglove::schemas::ImageAnnotationsChannel>
