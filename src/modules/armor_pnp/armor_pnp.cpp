...
struct ArmorPnp::Impl {
  ...
  [[nodiscard]] ArmorPnpAttempt Solve(std::span<const cv::Point2f, 4> image_corners,
                                      ...
                                      const frame::CameraModel& camera_model, PnpInputSource source,
...
ArmorPnpAttempt ArmorPnp::Impl::Solve(std::span<const cv::Point2f, 4> image_corners,
                                      ...
                                      const frame::CameraModel& camera_model, PnpInputSource source,
                                      std::size_t input_index, std::uint8_t label) const {
  return detail::SolveIppe(config, image_corners, type, camera_model, source, input_index, label);
}
...
ArmorPnpFrameResult ArmorPnp::Impl::ProcessFrame(
  ...
  if (simulation_data) {
    for (std::size_t index = 0; index < simulation_data->armors.size(); ++index) {
      const auto& armor = simulation_data->armors[index];
      const auto pixels = detail::ProjectVisibleTruth(armor, camera_model, kinematics);
      if (!pixels)
        continue;
      visible_truth.push_back({&armor, *pixels});
      auto attempt = Solve(*pixels, armor.type, camera_model, PnpInputSource::GROUND_TRUTH, index,
                           armor.label);
      if (attempt.estimate)
        detail::AddTruthErrors(*attempt.estimate, armor, kinematics, *pixels);
  ...
  for (std::size_t index = 0; index < detections.size(); ++index) {
    ...
    auto detection_attempt =
        Solve(final_corners, ArmorTypeForLabel(detection.label), camera_model,
    ...
    if (best_truth < visible_truth.size()) {
      ...
      if (detection_attempt.estimate)
        detail::AddTruthErrors(*detection_attempt.estimate, *truth.armor, kinematics, truth.pixels);
    }
  ...
  metrics.RecordAttempts(result.attempts, sequence);
  metrics.PopulateSnapshot(sequence, result);
  return result;
}
...
ArmorPnpAttempt ArmorPnp::Solve(std::span<const cv::Point2f, 4> image_corners,
                                geometry::ArmorType type, const frame::CameraModel& camera_model,
                                PnpInputSource source, std::size_t input_index,
                                std::uint8_t label) const {
  return impl_->Solve(image_corners, type, camera_model, source, input_index, label);
}
...
ArmorPnpFrameResult ArmorPnp::ProcessFrame(std::uint64_t sequence,
                                           ...
                                           std::span<const ArmorDetection> detections,
                                           std::span<const CornerRefinementResult> refinements) {
  return impl_->ProcessFrame(sequence, camera_model, kinematics, simulation_data, detections,
                             refinements);
}
