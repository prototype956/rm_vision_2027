...
class ArmorPnp final {
 public:
   ...
   *
   * refinements 与 detections 数量不一致时返回空结果且不更新统计。
   *
   * @param sequence 当前采集周期内的帧序号。
   * @param camera_model 与输入角点同帧的相机模型。
   * @param kinematics 与输入角点同帧的平台运动学。
   * @param simulation_data 可选的同帧仿真真值。
