...
struct UvlObservation {
  ...
  const frame::SpatialFrameView* geometry{nullptr};
};
...
[[nodiscard]] std::array<UvlObservation, 2> MakeUvlObservations(
    const std::array<cv::Point2f, 4>& corners, const frame::SpatialFrameView& geometry,
    double armor_tilt_rad, geometry::ArmorType type, int slot);

/** @brief 从按图像上下排序的单根灯条端点生成独立 UVL 观测。 */
[[nodiscard]] UvlObservation MakeStandaloneUvlObservation(cv::Point2f top, cv::Point2f bottom,
                                                          const frame::SpatialFrameView& geometry,
                                                          double armor_tilt_rad,
                                                          geometry::ArmorType type, int slot,
                                                          bool left);
