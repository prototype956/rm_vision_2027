...
ProjectedKinematics ProjectKinematics(const modules::ControlInputSnapshot& input,
  ...
  if (!feedback.valid || feedback.timestamp > now || !std::isfinite(feedback.yaw) ||
      !std::isfinite(feedback.pitch)) {
    ProjectedKinematics result;
    result.dt_s = DT_S;
...
ControlSession::ControlSession(FireControlConfig config, GimbalTrajectoryPlannerConfig planner)
    : fire_control_(config, planner),
      feedback_estimator_(planner.max_yaw_velocity_rad_s, planner.max_pitch_velocity_rad_s),
      PLANNER_DT_S(planner.dt_s) {}
void ControlSession::Reset() {
  ...
  last_successful_command_index_ = 1;
  last_successful_target_distance_m_ = -1;
  last_successful_plan_time_ = {};
}
...
void ControlSession::RememberCommand(const hal::GimbalCommand& command) {
  ...
  // 通信时间戳可能回拨或重复；容量上限保证长时会话不会无限积累历史。
  constexpr std::size_t MAX_COMMAND_HISTORY = 1024;
  while (!sent_commands_.empty() &&
         (sent_commands_.size() > MAX_COMMAND_HISTORY ||
          (command.timestamp_ns > sent_commands_.front().timestamp_ns &&
...
ControlStepResult ControlSession::Step(const ControlInputSnapshot& sample,
                                       const hal::GimbalActuatorTelemetry& actuator,
                                       bool sink_healthy, std::chrono::steady_clock::time_point now,
                                       std::uint64_t system_now_ns,
                                       std::optional<PolicyDecision> decision) {
  return StepImpl(sample, actuator, sink_healthy, now, system_now_ns, decision, {});
}

ControlStepResult ControlSession::StepWithPolicy(const ControlInputSnapshot& sample,
                                                 const hal::GimbalActuatorTelemetry& actuator,
                                                 bool sink_healthy,
                                                 std::chrono::steady_clock::time_point now,
                                                 std::uint64_t system_now_ns,
  ...
  if (!policy)
    throw std::invalid_argument("external policy must be callable");
  return StepImpl(sample, actuator, sink_healthy, now, system_now_ns, std::nullopt, policy);
}

ControlStepResult ControlSession::StepImpl(
    const ControlInputSnapshot& sample, const hal::GimbalActuatorTelemetry& actuator,
    bool sink_healthy, std::chrono::steady_clock::time_point now, std::uint64_t system_now_ns,
    std::optional<PolicyDecision> decision, const ControlPolicy& policy) {
  if (awaiting_ack_)
    throw std::logic_error("acknowledge the previous control cycle before stepping");
  if (round_ && *round_ != sample.prediction.source_round_id)
    Reset();
  round_ = sample.prediction.source_round_id;
  if (last_step_time_ && now <= *last_step_time_)
    throw std::invalid_argument("control time must increase within a round");
  last_step_time_ = now;
  ...
  // 不健康的通信不能向策略开放射击动作；跟踪状态仍可用于诊断。
  input.external_control_enabled = input.external_control_enabled && sink_healthy;
  auto result = fire_control_.Step(input, FEEDBACK, now, system_now_ns, decision, policy);
  result.output.external_control_enabled = sample.external_control_enabled;
  ...
  output.command.source_capture_timestamp_ns =
      sample.prediction.source_capture_timestamp_ns.value_or(0);
  result.session_owner = this;
  result.cycle_id = ++cycle_;
  awaiting_ack_ = true;
  return result;
}
void ControlSession::AcknowledgePublication(ControlStepResult& result, bool send_succeeded,
                                            std::chrono::steady_clock::time_point now) {
  if (!awaiting_ack_ || result.session_owner != this || result.cycle_id != cycle_)
    throw std::logic_error("stale or duplicate publication acknowledgement");
  if (!last_step_time_ || now < *last_step_time_)
    throw std::invalid_argument("publication acknowledgement predates the control step");
  awaiting_ack_ = false;
  auto& output = result.output;
