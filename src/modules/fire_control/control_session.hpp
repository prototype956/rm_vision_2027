...
class ControlSession final {
 public:
  ...
  [[nodiscard]] ControlStepResult Step(const ControlInputSnapshot& input,
  ...
  /** @brief 使用当前融合反馈和投影位姿调用一次策略，再求解并返回待发布命令。 */
  [[nodiscard]] ControlStepResult StepWithPolicy(const ControlInputSnapshot& input,
                                                 const hal::GimbalActuatorTelemetry& actuator,
                                                 bool sink_healthy,
 ...
 private:
  [[nodiscard]] ControlStepResult StepImpl(const ControlInputSnapshot& input,
                                           const hal::GimbalActuatorTelemetry& actuator,
                                           bool sink_healthy,
                                           std::chrono::steady_clock::time_point now,
                                           std::uint64_t command_timestamp_ns,
