...
图像相交的装甲；原始和精修框不再绘制角点身份文字，详细角点数据保留在统计消息中。
严格遮挡仍需仿真渲染端提供深度或可见性 ID。

按 2/4/6/8/10 m 和 0/15/30/45° 采集 MCAP。`/vision/pnp/stats.summary` 每 100 帧原子更新
真值、原始检测、精修成功样本和含回退精修路径的累计 P50/P95；`summary_sequence` 标识
全局与全部分组共同使用的统计快照。距离、观察角和大小装甲分组均使用匹配真值，另提供
精修回退原因和两条检测链的求解成功/候选切换计数。逐目标记录有符号角点偏差、相机系
XYZ 误差、有符号深度误差和两个 IPPE 候选的 RMSE 间隔。真值角点链要求重投影 RMSE ≤0.1 px、
位置误差 ≤5 mm；非正面退化视角的姿态误差 ≤0.5°。只有检测角点误差与位姿误差明显相关时，
才进入 PCA 灯条精修。

梯度精修采用整块原子接受：四个端点全部有效时才求解精修 PnP，任何端点失败、四边形非法
或精修 PnP 无解时直接复制原始 PnP 结果，禁止部分精修端点与原始端点组成混合四边形。
