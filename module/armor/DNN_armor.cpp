...
void DNN_Dectect::Detect(cv::Mat& src_img, DNN_Model& model) {
  ...
  // output = model.session.Run(Ort::RunOptions{nullptr}, input_names.data(), ort_input_tensor, 1,
  // output_names.data(), 1);
...
void DNN_Detect_CV::Detect(cv::Mat& src_img, DNN_Model_CV& model) {
  float r = min(model.DNN_Config_.width / (src_img.cols * 1.0),
                model.DNN_Config_.height / (src_img.rows * 1.0));
  cv::Mat img = imgResize(src_img, model);
  for (int i = 0; i < img.rows; i++) {
    float* pdata = (float*)(img.data + i * img.step);
    for (int j = 0; j < img.cols; ++j) {
      pdata[0] =
          (pdata[2] / 255.0 - model.DNN_Config_.mean_vals[0]) / model.DNN_Config_.norm_vals[0];
      pdata[1] =
          (pdata[1] / 255.0 - model.DNN_Config_.mean_vals[1]) / model.DNN_Config_.norm_vals[1];
      pdata[2] =
          (pdata[0] / 255.0 - model.DNN_Config_.mean_vals[2]) / model.DNN_Config_.norm_vals[2];
      pdata += 3;
    }
  }
...
cv::Mat DNN_Detect_CV::imgResize(cv::Mat& src_img, DNN_Model_CV& model) {
  ...
  cv::Mat out(model.DNN_Config_.height, model.DNN_Config_.width, CV_8UC3,
