...
class Detector {
 public:
   ...
   */
  void initialPredictionData(const float _gyro_speed, const float _bullet_velocity,
  ...
  uart::Write_Data::node returnArmorCenter(const int _num) {
    if (armor_.size()) {
      return (uart::Write_Data::node){int(armor_[_num].armor_rect.center.x),
                                      int(armor_[_num].armor_rect.center.y)};
    } else {
