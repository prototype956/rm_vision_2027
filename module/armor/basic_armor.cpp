...
bool Detector::findLight(cv::Mat& src_img, int my_color) {
  ...
  cv::findContours(bin_color_img, contours, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_NONE);
  ...
  if (light_config_.light_edit == 1 && !light_config_.light_edit) {
    ...
    cv::createTrackbar("angle_min", window_name, &light_config_.angle_min, 1800, NULL);
    cv::createTrackbar("angle_max", window_name, &light_config_.angle_max, 1800, NULL);

    cv::createTrackbar("perimeter_min", window_name, &light_config_.perimeter_min, 100000, NULL);
    cv::createTrackbar("perimeter_max", window_name, &light_config_.perimeter_max, 100000, NULL);

    cv::createTrackbar("ratio_w_h_min", window_name, &light_config_.ratio_w_h_min, 1000, NULL);
    cv::createTrackbar("ratio_w_h_max", window_name, &light_config_.ratio_w_h_max, 1000, NULL);
  ...
  for (size_t i = 0; i != contours.size(); ++i) {
    ...
    if (perimeter < light_config_.perimeter_min || perimeter > light_config_.perimeter_max ||
        contours[i].size() < 5) {
      continue;
    }
    ...
    // 判断灯条的条件
    if (box.angle < light_config_.angle_max && box.angle > light_config_.angle_min &&
      ...
      if (light_config_.light_draw == 1 || light_config_.light_edit == 1) {
        cv::Point2f vertex[4];
        box.points(vertex);
        cv::putText(draw_img_, std::to_string(int(box.angle)),
                    {int(vertex[1].x), int(vertex[1].y - 5)}, cv::FONT_HERSHEY_SIMPLEX, 1,
                    cv::Scalar(0, 255, 255));
...
bool Detector::colorCheck(cv::RotatedRect& rect, cv::Mat& src_img, uart::Color my_color) {
  ...
  if (rect_.tl().x > src_img.cols || rect_.br().x > src_img.cols || rect_.tl().y > src_img.rows ||
      rect_.br().y > src_img.rows || rect_.tl().x <= 0 || rect_.br().x <= 0 || rect_.tl().y <= 0 ||
      rect_.br().y <= 0) {
    return false;
  }
  ...
  // tools::Tools::drawDiagram("B channel", B_channel_light, armor_config_.window_scale, (B_val
  // * 1.0 / 255), "time", "Avg B", 255, cv::Scalar(255, 0, 0)); tools::Tools::drawDiagram("G
  // channel", G_channel_light, armor_config_.window_scale, (G_val * 1.0 / 255), "time", "Avg G",
  // 255, cv::Scalar(0, 255, 0)); tools::Tools::drawDiagram("R channel", R_channel_light,
  // armor_config_.window_scale, (R_val * 1.0 / 255), "time", "Avg R", 255, cv::Scalar(0, 0, 255));
  fmt::print("[{}] B {} G {} R {}\n", idntifier_green, B_val, G_val, R_val);
  switch (my_color) {
    case uart::RED:
      if (B_val >= image_config_.b_blue_th_min && B_val <= image_config_.b_blue_th_max &&
          G_val >= image_config_.g_blue_th_min && G_val <= image_config_.g_blue_th_max &&
          R_val >= image_config_.r_blue_th_min && R_val <= image_config_.r_blue_th_max) {
      ...
      break;
    case uart::BLUE:
      if (B_val >= image_config_.b_red_th_min && B_val <= image_config_.b_red_th_max &&
          G_val >= image_config_.g_red_th_min && G_val <= image_config_.g_red_th_max &&
          R_val >= image_config_.r_red_th_min && R_val <= image_config_.r_red_th_max) {
  ...
  //  cv::putText(src_img, "B " + std::to_string(B_val), {20, 20}, cv::FONT_HERSHEY_SIMPLEX, 1,
  //  cv::Scalar(255, 255, 0)); cv::putText(src_img, "G " + std::to_string(B_val), {120, 20},
  //  cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(255, 255, 0)); cv::putText(src_img, "R " +
  //  std::to_string(B_val), {220, 20}, cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(255, 255, 0));
    ...
    if (fittingArmor()) {
      ...
      if (armor_config_.armor_draw == 1 || light_config_.light_draw == 1 ||
        ...
        if (armor_config_.debug_mode == 1) {
          ...
          // tools::Tools::drawDiagram("diagram", armor_x_point, armor_config_.window_scale,
          // (armor_data_.armor_rect.center.x * 1.0 / _src_img.cols), "time", "armor_x",
          // _src_img.cols);
        }
...
bool Detector::sentryMode(const cv::Mat& _src_img, const uart::Receive_Data _receive_data) {
  ...
  if (sentry_cnt_ == 0) {
    // 更新哨兵预测方向及预测量
    initialPredictionData(_receive_data.pitch, _receive_data.bullet_velocity, _receive_data.yaw);
    ...
    if (findLight(draw_img_, /*_receive_data.my_color*/ uart::BLUE)) {
      if (fittingArmor()) {
        ...
        lost_cnt_ = 10;
        // 限制最大预测位置
        if (abs(forecast_pixels_) > armor_[0].armor_rect.size.width * forecast_max_size_ * 0.1) {
          forecast_pixels_ = armor_[0].armor_rect.size.width * forecast_max_size_ * 0.1;
        }
        // 添加预测值
        ...
        cv::rectangle(draw_img_, armor_[0].armor_rect.boundingRect(), cv::Scalar(0, 255, 0), 3, 8);
        if (armor_config_.armor_draw == 1 || light_config_.light_draw == 1 ||
            armor_config_.armor_edit == 1 || light_config_.light_edit == 1) {
          if (armor_config_.debug_mode == 1) {
            tools::Tools::imWindow(window_name, draw_img_, armor_config_.window_scale);
...
void Detector::initialPredictionData(const float _gyro_speed_data, const float _bullet_velocity,
  ...
  if (num_cnt_ % 2 == 0) {
    ...
    // 计算预测角度 角速度 * 时间
    forecast_angle_ =
        static_cast<float>(actual_depth_ * 0.001 / _bullet_velocity) * _gyro_speed_data;

    // 计算像素点个数
    forecast_pixels_ =
        abs(camera_focal_ * tan(forecast_angle_ * CV_PI / 180) * forecast_size_ * 100 / 4.8);

    kalman_.setParam(R_, Q_, 1.0);

    forecast_pixels_ =
        (last_last_forecast_pixels_ + last_forecast_pixels_ + forecast_pixels_) * 0.333;
...
void Detector::finalArmor() {
  ...
  if (armor_.size() == 1) {
    ...
    std::sort(armor_.begin(), armor_.end(),
              [](Armor_Data _a, Armor_Data _b) { return _a.distance_center < _b.distance_center; });
...
bool Detector::fittingArmor() {
  if (armor_config_.armor_edit && !armor_config_.armor_trackbar) {
    ...
    cv::createTrackbar("light_width_aspect_min", window_name, &armor_config_.light_width_ratio_min,
                       100, NULL);
    cv::createTrackbar("light_width_ratio_max", window_name, &armor_config_.light_width_ratio_max,
                       100, NULL);
    cv::createTrackbar("light_y_different", window_name, &armor_config_.light_y_different, 100,
                       NULL);
    cv::createTrackbar("light_height_different", window_name, &armor_config_.light_height_different,
                       100, NULL);
    cv::createTrackbar("armor_angle_different", window_name, &armor_config_.armor_angle_different,
                       100, NULL);
    cv::createTrackbar("small_armor_aspect_min", window_name, &armor_config_.small_armor_aspect_min,
                       100, NULL);
    cv::createTrackbar("armor_type_th", window_name, &armor_config_.armor_type_th, 100, NULL);
    cv::createTrackbar("big_armor_aspect_max", window_name, &armor_config_.big_armor_aspect_max,
                       100, NULL);
  ...
  for (size_t i = 0; i != light_.size(); ++i) {
    for (size_t j = i + 1; j != light_.size(); ++j) {
      ...
      // 装甲板倾斜弧度
      float error_angle = atan((light_[light_right].center.y - light_[light_left].center.y) /
      ...
      if (error_angle < 20.f) {
        ...
        if (lightJudge(light_left, light_right)) {
          ...
          if (averageColor() < 30) {
            // 储存装甲板
            // cv::line(draw_img_, armor_data_.right_light, vertex[(l + 1) % 4], cv::Scalar(0, 255,
            // 255), 3, 8); std::cout << armor_data_.width * 1.0 / armor_data_.height << '\n';

            if (armor_data_.width * 1.0 / armor_data_.height > 1.0 &&
                armor_data_.width * 1.0 / armor_data_.height < 5.0) {
              // tools::Tools::drawDiagram("W/H", W_H_armor, armor_config_.window_scale,
              // (armor_data_.width * 1.0 / armor_data_.height / 12.0), "time", "W/H", 12,
              // cv::Scalar(255, 0, 0));
              armor_.push_back(armor_data_);
              if (armor_config_.armor_draw == 1 || armor_config_.armor_edit == 1) {
                cv::Point2f vertices[4];
                armor_data_.armor_rect.points(vertices);
                rectangle(draw_img_, armor_data_.armor_rect.boundingRect(), cv::Scalar(255, 200, 0),
                          5, 8);
                cv::putText(draw_img_, std::to_string(int(armor_data_.height * armor_data_.width)),
                            {int(vertices[1].x), int(vertices[1].y - 5)}, cv::FONT_HERSHEY_SIMPLEX,
                            1, cv::Scalar(255, 255, 0));
              }
            }
          }
        }
...
bool Detector::lightJudge(const int i, const int j) {
  armor_data_.left_light_height = MAX(light_[i].size.height, light_[i].size.width);
  armor_data_.left_light_width = MIN(light_[i].size.height, light_[i].size.width);
  armor_data_.right_light_height = MAX(light_[j].size.height, light_[j].size.width);
  armor_data_.right_light_width = MIN(light_[j].size.height, light_[j].size.width);
  armor_data_.light_height_aspect = armor_data_.left_light_height / armor_data_.right_light_height;
  armor_data_.light_width_aspect = armor_data_.left_light_width / armor_data_.right_light_width;
  ...
  if (armor_data_.light_height_aspect < armor_config_.light_height_ratio_max * 0.1 &&
    ...
    if (fabs(armor_data_.left_light.center.y - armor_data_.right_light.center.y) <
      ...
      if (fabs(armor_data_.left_light.size.height - armor_data_.right_light.size.height) <
        ...
        armor_data_.width =
            getDistance(armor_data_.left_light.center, armor_data_.right_light.center);

        armor_data_.aspect_ratio = armor_data_.width / (MAX(armor_data_.left_light.size.height,
        ...
        if (fabs(armor_data_.left_light.angle - armor_data_.right_light.angle) <
          ...
          armor_data_.distance_center =
              getDistance(armor_data_.armor_rect.center, cv::Point(draw_img_.cols, draw_img_.rows));
          // 小装甲板比例范围
...
int Detector::averageColor() {
  ...
  cv::putText(draw_img_, std::to_string(average_intensity),
              {int(vertices[0].x), int(vertices[0].y + 100)}, cv::FONT_HERSHEY_SIMPLEX, 1,
              cv::Scalar(255, 255, 0));
...
void Detector::runImage(const cv::Mat& _src_img, const int _my_color) {
  ...
  switch (image_config_.method) {
    case 0:
      bin_color_img = fuseImage(grayPretreat(_src_img, _my_color), bgrPretreat(_src_img, _my_color),
                                whitePretreat(_src_img));
      break;
    default:
      bin_color_img = fuseImage(grayPretreat(_src_img, _my_color), hsvPretreat(_src_img, _my_color),
                                whitePretreat(_src_img));
      break;
  }
}

cv::Mat Detector::fuseImage(const cv::Mat _bin_gray_img, const cv::Mat _bin_color_img,
                            const cv::Mat _white_img) {
...
inline cv::Mat Detector::whitePretreat(const cv::Mat& _src_img) {
  ...
  cv::threshold(gray_white_img_, white_img_, image_config_.white_armor_color_th, 255,
                cv::THRESH_BINARY);
  ...
  switch (_my_color) {
    case uart::RED:
      ...
      if (image_config_.gray_edit && !image_config_.gray_trackbar) {
        ...
        cv::createTrackbar("blue_gray_th", window_name, &image_config_.blue_armor_gray_th, 255,
                           NULL);
        cv::imshow(window_name, gray_trackbar_);
      }
      cv::threshold(gray_img_, bin_gray_img, image_config_.blue_armor_gray_th, 255,
                    cv::THRESH_BINARY);
      break;
    case uart::BLUE:
      ...
      cv::threshold(gray_img_, bin_gray_img, image_config_.red_armor_gray_th, 255,
                    cv::THRESH_BINARY);
      break;
    default:
      ...
      cv::threshold(gray_img_, bin_red_gray_img, image_config_.red_armor_gray_th, 255,
                    cv::THRESH_BINARY);
      cv::threshold(gray_img_, bin_blue_gray_img, image_config_.blue_armor_gray_th, 255,
                    cv::THRESH_BINARY);
  ...
  switch (_my_color) {
    case uart::RED:
      ...
      if (image_config_.color_edit && !image_config_.bgr_trackbar) {
        ...
        cv::namedWindow(window_name);
        cv::createTrackbar("blue_color_th", window_name, &image_config_.blue_armor_color_th, 255,
                           NULL);
        cv::imshow(window_name, bgr_trackbar_);
      }

      cv::threshold(bin_color_img, bin_color_img, image_config_.blue_armor_color_th, 255,
                    cv::THRESH_BINARY);
      cv::threshold(bin_color_green_img, bin_color_green_img, image_config_.green_armor_color_th,
    ...
    case uart::BLUE:
      ...
      if (image_config_.color_edit && !image_config_.bgr_trackbar) {
        image_config_.bgr_trackbar = true;
        cv::namedWindow(window_name);
        cv::createTrackbar("red_color_th", window_name, &image_config_.red_armor_color_th, 255,
                           NULL);
        cv::imshow(window_name, bgr_trackbar_);
      }

      cv::threshold(bin_color_img, bin_color_img, image_config_.red_armor_color_th, 255,
                    cv::THRESH_BINARY);
      cv::threshold(bin_color_green_img, bin_color_green_img, image_config_.green_armor_color_th,
                    255, cv::THRESH_BINARY);
      cv::dilate(bin_color_green_img, bin_color_green_img, ele_);
    ...
    default:
      ...
      cv::threshold(bin_blue_color_img, bin_blue_color_img, image_config_.blue_armor_color_th, 255,
                    cv::THRESH_BINARY);
      cv::threshold(bin_blue_green_img, bin_blue_green_img, image_config_.green_armor_color_th, 255,
                    cv::THRESH_BINARY);
      cv::threshold(bin_red_color_img, bin_red_color_img, image_config_.red_armor_color_th, 255,
                    cv::THRESH_BINARY);
      cv::threshold(bin_red_green_img, bin_red_green_img, image_config_.green_armor_color_th, 255,
                    cv::THRESH_BINARY);
