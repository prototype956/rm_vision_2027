...
class model {
 public:
  ...
  @param: onnx_model_path, the path of the modle on your machine, downloadable at
  ...
  */
  inline explicit model(std::string onnx_model_path,
                        const cv::Size& input_size = cv::Size(28, 28)) {
  ...
  @param:  median_blur_kernel_size, define the kernel size of median blur pre-processing, default to
  int 5, set 0 to disable
  @param:  hsv_lowerb, the lower range for hsv image, pixels inside the range equals to 1, otherwise
  equals to 0, default is the cv::Scalar() default
  @param:  hsv_upperb, the upper range for hsv image, pixels inside the range equals to 1, otherwise
  equals to 0, default is the cv::Scalar() default
  @param:  probability_threshold, the min probability of considerable probability to iterate,
  determined by the model, mnist-8.onnx has the output array from -1e5 to 1e5, default is 0
  @return: max_probability_idx, the most probable digit classified from input image in int type, -1
  means all the probability is out of the threahold
  */
  inline int inferring(const cv::Mat& hsv_input, const int median_blur_kernel_size = 3,
                       float probability_threshold = 0,
                       cv::Mat image_input = cv::Mat::zeros(cv::Size(255, 0), CV_8UC3)) {
    ...
    cv::inRange(Image_output,
                cv::Scalar(param_ocr.h_min_num, param_ocr.s_min_num, param_ocr.v_min_num),
                cv::Scalar(255, 255, 255), Image_output);
    ...
    opencv_net.forward().forEach<float>(
        [&probability_threshold, &max_probability, &max_probability_idx, &i](
    ...
    if (!Image_showput.empty()) {
      cv::putText(Image_showput, to_string(max_probability_idx), cv::Point(100, 100),
                  cv::FONT_HERSHEY_DUPLEX, 2, cv::Scalar(255, 255, 255), 2, cv::LINE_AA);
