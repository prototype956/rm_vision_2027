...
class RoI {
 public:
   ...
   * @return cv::Mat   返回截取后的图片
   */
  cv::Mat cutRoIRotatedRect(const cv::Mat& _input_img, const cv::RotatedRect& _rect) {
    int roi_w = MAX(_rect.size.width, _rect.size.height);
    int roi_h = MIN(_rect.size.width, _rect.size.height);
    cv::RotatedRect r_rect = cv::RotatedRect(_rect.center, cv::Size(roi_w, roi_h), _rect.angle);
    ...
    cv::warpPerspective(_input_img, roi_img_r_rect, warpMatrix, roi_img_r_rect.size(),
  ...
  inline cv::Point2d coordMapping(const cv::Point& _input_point) {
    return cv::Point(_input_point.x + tl_.x, _input_point.y + tl_.y);
  }
  /**
   * @brief Rect 坐标转换
   *
  ...
  inline cv::RotatedRect rotatedRectMapping(const cv::RotatedRect& _input_r_rect) {
    return cv::RotatedRect(coordMapping(_input_r_rect.center), _input_r_rect.size,
                           _input_r_rect.angle);
  }
