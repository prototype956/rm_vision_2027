...
cv::Rect RoI::makeRectSafeFixed(const cv::Mat& _input_img, const cv::RotatedRect& _r_rect) {
  int width = _r_rect.boundingRect().width;
  int height = _r_rect.boundingRect().height;

  cv::Point tl = cv::Point(_r_rect.center.x - (width * 0.5f), _r_rect.center.y - (height * 0.5f));
  // 限制 ROI 出界条件
  if (tl.x < 0) {
    tl.x = 0;
  }
...
cv::Rect RoI::makeRectSafeTailor(const cv::Mat& _input_img, const cv::RotatedRect& _r_rect) {
  int width = _r_rect.boundingRect().width;
  int height = _r_rect.boundingRect().height;
...
cv::Rect RoI::makeRectSafeTailor(const cv::Mat& _input_img, const cv::Rect& _r_rect) {
...
cv::Rect RoI::makeRectSafeThird(const cv::Mat& _input_img, const cv::RotatedRect& _r_rect) {
  int width = _r_rect.boundingRect().width;
  int height = _r_rect.boundingRect().height;
...
cv::Mat RoI::returnROIResultMat(const cv::Mat& _input_img) {
  if (!roi_armor_data_.last_armor_success) {
    ...
    // 更新上一帧 ROI 参数
    roi_armor_data_.last_rect = makeRectSafeThird(_input_img, roi_armor_data_.last_roi_armor_rect);
    return _input_img(roi_armor_data_.last_rect);
  }
}
}  // namespace basic_roi
