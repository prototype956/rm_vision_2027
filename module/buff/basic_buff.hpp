...
auto final_target_yellow =
    fmt::format(fg(fmt::color::yellow) | fmt::emphasis::bold, "final_target");
...
class Detector {
 public:
   ...
   */
  void runTask(cv::Mat& _input_img, const uart::Receive_Data& _receive_info,
 ...
 private:
   ...
   */
  void imageProcessing(cv::Mat& _input_img, cv::Mat& _output_img, const int& _my_color,
...
// BGR
 ...
 private:
   ...
   */
  void findTarget(cv::Mat& _input_dst_img, cv::Mat& _input_bin_img,
                  std::vector<abstract_target::Target>& _target_box,
                  const new_buff::Check_Moudle& check_moudle);
  void findTarget_new(cv::Mat& _input_dst_img, cv::Mat& _input_bin_img,
                      std::vector<abstract_target::Target>& _target_box,
                      const new_buff::Check_Moudle& check_moudle);
 ...
 private:
   ...
   */
  cv::Point2f findCircleR(cv::Mat& _input_src_img, cv::Mat& _input_bin_img, cv::Mat& _dst_img,
 ...
 private:
  // 卡尔曼滤波器
  // basic_kalman::firstKalman buff_filter_ = basic_kalman::firstKalman(0.01f, 0.03f, 1.f, 0.f,
  // 0.f);
 ...
 private:
   ...
   */
  void calculateTargetPointSet(const float& _predict_quantity, const cv::Point2f& _final_center_r,
                               std::vector<cv::Point2f>& _target_2d_point, cv::Mat& _input_dst_img,
                               const bool& _is_find_target);
 ...
 private:
  ...
  basic_pnp::PnP buff_pnp_ =
      basic_pnp::PnP(fmt::format("{}{}", CONFIG_FILE_PATH, "/camera/mv_camera_config_407.xml"),
   ...
   * @return cv::Point2f
   */
  cv::Point2f angleCalculation(const cv::Point2f& _target_center, const float& _unit_pixel_length,
                               const cv::Size& _image_size, const float& _focal_length);
