...
void buff::getcontour(cv::Mat imgDil, cv::Mat imgDil2, cv::Mat& img_src,
                      new_buff::Check_Moudle moudle)  // 判断轮廓
{
  int objCor, cnt_obj;
  cnt_obj = 0;
    ...
    if (area > 10)  // 排除小黑圆点的干扰
    {
      ...
      if (objCor > 4 && area < 4800) {
        if (moudle == INACTION_MODE) {
          // if(area>300){
          // cv::rectangle(img_src, boundRect[i].tl() ,boundRect[i].br(),cv::Scalar(200, 255, 125),
          // 2, 8);
          //}
          ...
          if (fabs(fabs(returnDistance((P_a + P_b) / 2, circles) / max_dist - 56.0 / 25.0)) <
              0.2 /*&&dis>60.0*/) {
            armor_check tmp;
          ...
          cv:
            ...
            tmp.cord[0].x =
                ((boundRect[i].tl().x + boundRect[i].br().x) / 2 - circles.x) * 50.0 / 56.0 +
                circles.x;
            tmp.cord[0].y =
                ((boundRect[i].tl().y + boundRect[i].br().y) / 2 - circles.y) * 50.0 / 56.0 +
                circles.y;
            tmp.cord[1].x =
                ((boundRect[i].tl().x + boundRect[i].br().x) / 2 - circles.x) * 54.0 / 56.0 +
                circles.x;
            tmp.cord[1].y =
                ((boundRect[i].tl().y + boundRect[i].br().y) / 2 - circles.y) * 54.0 / 56.0 +
                circles.y;
            tmp.cord[2].x =
                ((boundRect[i].tl().x + boundRect[i].br().x) / 2 - circles.x) * 56.0 / 56.0 +
                circles.x;
            tmp.cord[2].y =
                ((boundRect[i].tl().y + boundRect[i].br().y) / 2 - circles.y) * 56.0 / 56.0 +
                circles.y;
            tmp.cord[3].x =
                ((boundRect[i].tl().x + boundRect[i].br().x) / 2 - circles.x) * 53.0 / 56.0 +
                circles.x;
            tmp.cord[3].y =
                ((boundRect[i].tl().y + boundRect[i].br().y) / 2 - circles.y) * 53.0 / 56.0 +
                circles.y;
            tmp.obj.x =
                ((boundRect[i].tl().x + boundRect[i].br().x) / 2 - circles.x) * 47.0 / 56.0 +
                circles.x;
            tmp.obj.y =
                ((boundRect[i].tl().y + boundRect[i].br().y) / 2 - circles.y) * 47.0 / 56.0 +
                circles.y;
            ...
            cv::putText(img_src, std::to_string(ratio), {boundRect[i].x, boundRect[i].y - 5}, 1,
                        cv::FONT_HERSHEY_PLAIN, cv::Scalar(0, 69, 255), 1);
            ...
            // cv::circle(img_src,{((boundRect[i].tl().x+boundRect[i].br().x)/2-circles.x)*48.0/56.0+circles.x,((boundRect[i].tl().y+boundRect[i].br().y)/2-circles.y)*48.0/56.0+circles.y},2,cv::Scalar(255,0,255),1);
               ...
               if(returnDistance(P_a,circles)<returnDistance(P_b,circles)){
                   P_re.x = (P_a.x-circles.x) * cos(PI/180*15) - (P_a.y-circles.y) * sin(PI/180*15)
           + circles.x; P_re.y = (P_a.x-circles.x) * sin(PI/180*15) + (P_a.y-circles.y) *
           cos(PI/180*15) + circles.y;
                   if(imgDil.at<uchar>((cv::Point)((P_re-circles)*0.5+circles))!=0){
                       P_re.x = (P_a.x-circles.x) * cos(PI/180*15*(-1.0)) - (P_a.y-circles.y) *
           sin(PI/180*15*(-1.0)) + circles.x; P_re.y = (P_a.x-circles.x) * sin(PI/180*15*(-1.0)) +
           (P_a.y-circles.y) * cos(PI/180*15*(-1.0)) + circles.y;
                   ...
                   P_re.x = (P_b.x-circles.x) * cos(PI/180*15) - (P_b.y-circles.y) * sin(PI/180*15)
           + circles.x; P_re.y = (P_b.x-circles.x) * sin(PI/180*15) + (P_b.y-circles.y) *
           cos(PI/180*15) + circles.y;
                   if(imgDil.at<uchar>((cv::Point)((P_re-circles)*0.5+circles))!=0){
                       P_re.x = (P_b.x-circles.x) * cos(PI/180*15*(-1.0)) - (P_b.y-circles.y) *
           sin(PI/180*15*(-1.0)) + circles.x; P_re.y = (P_b.x-circles.x) * sin(PI/180*15*(-1.0)) +
           (P_b.y-circles.y) * cos(PI/180*15*(-1.0)) + circles.y;
               ...
               if(abs((boundRect[i].br().x-boundRect[i].tl().x)*1.0/(boundRect[i].br().y-boundRect[i].tl().y)*1.0-1.0)<0.2){
                   ...
                   cv::circle(img_src,{tmp.cord[1].x,tmp.cord[1].y},1,cv::Scalar(100, 255, 125),2);
                   cv::circle(img_src,{tmp.cord[2].x,tmp.cord[2].y},1,cv::Scalar(100, 255, 125),2);
                   cv::rectangle(img_src, boundRect[i].tl() ,boundRect[i].br(),cv::Scalar(200, 255,
           125), 2, 8); cv::putText(img_src, std::to_string(area), { boundRect[i].x,boundRect[i].y -
           5 },1,cv::FONT_HERSHEY_PLAIN, cv::Scalar(0, 69, 255), 1);
                   tmp.d=returnDistance(boundRect[i].tl(),boundRect[i].br());
                    cv::putText(img_src, std::to_string(tmp.d), { tmp.obj.x,tmp.obj.y
           },1,cv::FONT_HERSHEY_PLAIN, cv::Scalar(0, 69, 255), 1); if(tmp.d<100.0){ tmp.isPass=true;
                       inaction_check.push_back(tmp);
                       cv::circle(img_src,{tmp.cord[2].x,tmp.cord[2].y},1,cv::Scalar(255, 255,
           125),2);
                   }
                   cv::imshow("inaction",img_src);
        ...
        /*
        if(moudle==INACTION_MODE&&area>150&&area<300&&area_rect>200&&area_rect<390&&hierarchy[i][3]>=
        1)
        //if(moudle==INACTION_MODE&&area>3200&&area<3700&&area_rect>3200&&area_rect<5300&&hierarchy[i][3]>=
        1)
        {
            objectType = "Circle";
            //fmt::print("[img process] Circle {} area {}\n",objCor,area);
            ...
            //rectangle(img_src, boundRect[i].tl(), boundRect[i].br(), cv::Scalar(0, 255, 0),
        5);//框出图形 tlx = boundRect[i].tl().x; tlx =(int)kal.run(boundRect[i].tl().x); tly =
        boundRect[i].tl().y; tly =(int)kal.run(boundRect[i].tl().y); brx = boundRect[i].br().x; brx
        =(int)kal.run(boundRect[i].br().x); bry = boundRect[i].br().y; bry
        =(int)kal.run(boundRect[i].br().y);
         ...
         cv::putText(img_src, std::to_string(imgDil.at<uchar>(inaction_check[j].cord[0])), {
        inaction_check[j].cord[0].x,inaction_check[j].cord[0].y-5},1,cv::FONT_HERSHEY_PLAIN,
        cv::Scalar(0, 69, 255), 1);   //}

            cv::putText(img_src, std::to_string(area_rect), { boundRect[i].x,boundRect[i].y - 5
        },1,cv::FONT_HERSHEY_PLAIN, cv::Scalar(0, 69, 255), 1); armor_last_rects = armor_now_rects;
            tan_angle = atan(abs(armor_now.y-circles.y)/abs(armor_now.x-circles.x));

            armor_now_rects = cv::RotatedRect(armor_now , cv::Size(abs(brx-tlx), abs(bry-tly)) ,
        tan_angle); cv::rectangle(img_src, armor_now_rects.boundingRect(),cv::Scalar(0, 255, 0), 2,
        8);
        }
        else
        {
        ...
        /*if(moudle==ACTION_MODE&&((area>1700&&area<1770))&&(hierarchy[i][3]>=
        0&&hierarchy[i][3]<=3))
        {
            ...
            //fmt::print("[img process] Circle {} area {}\n",objCor,area);
            drawContours(img, conPoly, i, cv::Scalar(255, 0, 255), 2);
            rectangle(img_src, boundRect[i].tl(), boundRect[i].br(), cv::Scalar(255, 255, 0),
        5);//框出图形 cv::putText(img_src, std::to_string(area), { boundRect[i].x,boundRect[i].y - 5
        },1,cv::FONT_HERSHEY_PLAIN, cv::Scalar(0, 69, 255), 1);
        }*/
        isfindcircleR = false;
        // cv::imshow("cir_img",imgDil);
        if (moudle == CIRCLE_MODE && area > 10 && area < 15 &&
            abs((boundRect[i].br().x - boundRect[i].tl().x) * 1.0 /
                    (boundRect[i].br().y - boundRect[i].tl().y) * 1.0 -
                1.0) < 0.3 &&
            area_rect > 10 && area_rect < 32 && hierarchy[i][3] == -1)
        // if(moudle==CIRCLE_MODE&&area>400&&area<500&&abs((boundRect[i].br().x-boundRect[i].tl().x)*1.0/(boundRect[i].br().y-boundRect[i].tl().y)*1.0-1.0)<0.3&&area_rect>500&&area_rect<1000/*&&hierarchy[i][3]==1*/)
        {
          ...
          cv::putText(img_src, std::to_string(area_rect), {boundRect[i].x, boundRect[i].y - 5}, 1,
                      cv::FONT_HERSHEY_PLAIN, cv::Scalar(0, 69, 255), 1);
          ...
          rectangle(img_src, boundRect[i].tl(), boundRect[i].br(), cv::Scalar(0, 255, 255),
                    2);  // 框出图形
        ...
        for (int j = 0; j < inaction_check.size(); ++j) {
          ...
          /*for(int k=0;k<contour.size();++k) {
              ...
              &&boundRect[k].br().x>inaction_check[j].cord.x&&boundRect[k].br().y>inaction_check[j].cord.y){
                  ...
                  //rectangle(img_src, boundRect[i].tl(), boundRect[i].br(), cv::Scalar(0, 255,
          255), 2); break;
              }
          }*/
          ...
          for (int k = 38; k <= 54; k++) {
            cv::circle(img_src,
                       {(inaction_check[j].obj.x - circles.x) * k * 1.0 / 47.0 + circles.x,
                        (inaction_check[j].obj.y - circles.y) * k * 1.0 / 47.0 + circles.y},
                       3, cv::Scalar(255, 0, 255), 3);
            col_cnt =
                col_cnt + imgDil.at<uchar>(
                              {(inaction_check[j].obj.x - circles.x) * k * 1.0 / 47.0 + circles.x,
                               (inaction_check[j].obj.y - circles.y) * k * 1.0 / 47.0 + circles.y});
          }
          cv::putText(img_src, std::to_string(col_cnt / (24.0 * 255.0)),
                      {inaction_check[j].obj.x, inaction_check[j].obj.y - 5}, 1,
                      cv::FONT_HERSHEY_PLAIN, cv::Scalar(0, 255, 255), 2);
          fmt::print("[check] {}\n", col_cnt);

          if (col_cnt / (16.0 * 255.0) < 0.29 ||
              returnDistance(inaction_check[j].obj, circles) < 43.0) {
            inaction_check[j].isPass = false;
            continue;
          }
          ...
          // cv::putText(img_src, std::to_string(imgDil.at<uchar>(inaction_check[j].cord[0])), {
          // 30,20 },1,cv::FONT_HERSHEY_PLAIN, cv::Scalar(0, 69, 255), 1); cv::putText(img_src,
          // std::to_string(imgDil.at<uchar>(inaction_check[j].cord[0])), {
          // inaction_check[j].cord[0].x,inaction_check[j].cord[0].y-5},1,cv::FONT_HERSHEY_PLAIN,
          // cv::Scalar(0, 69, 255), 1); cv::putText(img_src,
          // std::to_string(imgDil.at<uchar>(inaction_check[j].cord[1])), {
          // inaction_check[j].cord[1].x,inaction_check[j].cord[1].y-5},1,cv::FONT_HERSHEY_PLAIN,
          // cv::Scalar(0, 69, 255), 1); cv::putText(img_src,
          // std::to_string(imgDil.at<uchar>(inaction_check[j].cord[2])), {
          // inaction_check[j].cord[2].x,inaction_check[j].cord[2].y-5},1,cv::FONT_HERSHEY_PLAIN,
          // cv::Scalar(0, 69, 255), 1); cv::putText(img_src,
          // std::to_string(imgDil.at<uchar>(inaction_check[j].cord[3])), {
          // inaction_check[j].cord[3].x,inaction_check[j].cord[3].y-5},1,cv::FONT_HERSHEY_PLAIN,
          // cv::Scalar(0, 69, 255), 1);
          /*if(imgDil.at<uchar>(inaction_check[j].cord[0])+imgDil.at<uchar>(inaction_check[j].cord[1])+imgDil.at<uchar>(inaction_check[j].cord[2])+imgDil.at<uchar>(inaction_check[j].cord[3])==0){
              inaction_check[j].isPass=true;
              //cv::putText(img_src,
          std::to_string(imgDil.at<uchar>(inaction_check[j].cord[0])+imgDil.at<uchar>(inaction_check[j].cord[1])+imgDil.at<uchar>(inaction_check[j].cord[2])+imgDil.at<uchar>(inaction_check[j].cord[3])),
          { inaction_check[j].obj.x,inaction_check[j].obj.y-5},1,cv::FONT_HERSHEY_PLAIN,
          cv::Scalar(0, 69, 255), 1);
  ...
  for (int i = 0; i < inaction_check.size(); ++i) {
    if (inaction_check[i].isPass) {
      ...
      armor_now_rects = cv::RotatedRect(
          armor_now, cv::Size((int)inaction_check[i].d, (int)inaction_check[i].d), tan_angle);
      // cv::putText(img_src, std::to_string(inaction_check[i].d), {
      // inaction_check[i].obj.x,inaction_check[i].obj.y-5},1,cv::FONT_HERSHEY_PLAIN, cv::Scalar(0,
      // 69, 255), 1);
      cv::rectangle(img_src, armor_now_rects.boundingRect(), cv::Scalar(0, 200, 0), 2, 8);

      cv::putText(img_src, "Dectected", {10, 10}, 1, cv::FONT_HERSHEY_PLAIN,
  ...
  /*for(int i=0;i<armor.size();++i){
      ...
      if(now_r==0){
          ...
          cv::putText(img_src, std::to_string(armor[i].d), {
  armor[i].obj.x,armor[i].obj.y-5},1,cv::FONT_HERSHEY_PLAIN, cv::Scalar(0, 255, 255), 2);
...
cv::Point2f buff::stablePerdict(cv::Point2f& circle_r, cv::Mat& img) {
  ...
  object.x = (armor_now.x - circle_r.x) * cos(PI / 180 * 42) -
             (armor_now.y - circle_r.y) * sin(PI / 180 * 42) + circle_r.x;
  object.y = (armor_now.x - circle_r.x) * sin(PI / 180 * 42) +
             (armor_now.y - circle_r.y) * cos(PI / 180 * 42) + circle_r.y;
  ...
  cv::putText(img, std::to_string(center_dist), {(int)armor_now.x, (int)armor_now.y}, 1,
              cv::FONT_HERSHEY_PLAIN, cv::Scalar(0, 69, 255), 1);
...
void buff::predict(cv::Point2f& circle_r, cv::Mat& img) {
  ...
  fmt::print("[buff] sqrt:{}\n",
             sqrt(pow(circle_r.x - relay.x, 2) + pow(circle_r.y - relay.y, 2)) /
                 sqrt(pow(circle_r.x - armor_now.x, 2) + pow(circle_r.y - armor_now.y, 2)));
  alpha = 2 * acos(sqrt(pow(circle_r.x - relay.x, 2) + pow(circle_r.y - relay.y, 2)) /
                   sqrt(pow(circle_r.x - armor_now.x, 2) + pow(circle_r.y - armor_now.y, 2)));
    ...
    switch (status) {
      case MAX:
        if (abs(velocity - max_velocity) < 0.1 ||
            (now_velocity - last_velocity) * (velocity - now_velocity) < 0) {
        ...
        break;
      case MIN:
        if (abs(velocity - min_velocity) < 0.1 ||
            (last_velocity - now_velocity) * (now_velocity - velocity) < 0) {
          T = (cv::getTickCount() - timex) / cv::getTickFrequency();
          omega = 2 * PI / T;
          A = (max_velocity - min_velocity) / 2.0;
          fmt::print("[buff info] armor pass\n");
...
cv::Point2f buff::calculateCord(cv::Point2f& circle_r) {
  ...
  beta =
      -A / omega *
          (cos(omega *
                   ((cv::getTickCount() - timex) / cv::getTickFrequency() + gimbal_delay / 1000.0) +
               sign * PI / 4.0) -
