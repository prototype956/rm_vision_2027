...
void calibrate() {
...
#endif
    ret = cv::findChessboardCorners(gray, cv::Size(7, 7), corners,
  ...
  cv::FileStorage conf_file(fmt::format("{}/mv_camera_config_new.xml", SAVE_FILE_PATH),
...
void assess(cv::Mat& image) {
  cv::FileStorage camera_config(fmt::format("{}/mv_camera_config_337.xml", SAVE_FILE_PATH),
                                cv::FileStorage::READ);
...
void CalibrationEvaluate()  // 标定结束后进行评价  NOT FIXED
{
  ...
  double err = 0;
  double total_err = 0;
  // calibrateCamera(objRealPoint, corners, Size(imageWidth, imageHeight), intrinsic, distCoeffs,
  // rvecs, tvecs, 0);
  cv::FileStorage camera_config(fmt::format("{}/mv_camera_config_new.xml", SAVE_FILE_PATH),
                                cv::FileStorage::READ);
  camera_config["camera-matrix"] >> intrinsic;
  camera_config["distortion"] >> distCoeffs;
...
void calRealPoint(std::vector<std::vector<cv::Point3f>>& obj, int boardwidth, int boardheight,
  ...
  for (int rowIndex = 0; rowIndex < boardheight; rowIndex++) {
    for (int colIndex = 0; colIndex < boardwidth; colIndex++) {
      //  imgpoint.at<Vec3f>(rowIndex, colIndex) = Vec3f(rowIndex * squaresize, colIndex*squaresize,
      //  0);
