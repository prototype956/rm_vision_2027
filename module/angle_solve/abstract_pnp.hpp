...
class PnP {
 public:
  ...
  PnP() {
    ...
    static double r_data[] = {1, 0, 0, 0, cos(theta), sin(theta), 0, -sin(theta), cos(theta)};
    ...
    big_object_3d_.emplace_back(
        cv::Point3f(-pnp_config_.big_armor_width * 0.5, -pnp_config_.big_armor_height * 0.5, 0));
    big_object_3d_.emplace_back(
        cv::Point3f(pnp_config_.big_armor_width * 0.5, -pnp_config_.big_armor_height * 0.5, 0));
    big_object_3d_.emplace_back(
        cv::Point3f(pnp_config_.big_armor_width * 0.5, pnp_config_.big_armor_height * 0.5, 0));
    ...
    small_object_3d_.emplace_back(cv::Point3f(-pnp_config_.small_armor_width * 0.5,
                                              -pnp_config_.small_armor_height * 0.5, 0));
    small_object_3d_.emplace_back(
        cv::Point3f(pnp_config_.small_armor_width * 0.5, -pnp_config_.small_armor_height * 0.5, 0));
    small_object_3d_.emplace_back(
        cv::Point3f(pnp_config_.small_armor_width * 0.5, pnp_config_.small_armor_height * 0.5, 0));
    small_object_3d_.emplace_back(
        cv::Point3f(-pnp_config_.small_armor_width * 0.5, pnp_config_.small_armor_height * 0.5, 0));
    ...
    buff_object_3d_.emplace_back(
        cv::Point3f(-pnp_config_.buff_armor_width * 0.5, -pnp_config_.buff_armor_height * 0.5, 0));
    buff_object_3d_.emplace_back(
        cv::Point3f(pnp_config_.buff_armor_width * 0.5, -pnp_config_.buff_armor_height * 0.5, 0));
    buff_object_3d_.emplace_back(
        cv::Point3f(pnp_config_.buff_armor_width * 0.5, pnp_config_.buff_armor_height * 0.5, 0));
  ...
  cv::RotatedRect rectChangeRotatedrect(cv::Rect _rect) {
    cv::RotatedRect box = cv::RotatedRect((_rect.tl() + _rect.br()) / 2,
   ...
   */
  void drawCoordinate(cv::Mat& _draw_img, cv::Mat& _rvec, cv::Mat& _tvec, cv::Mat& _cameraMatrix,
                      cv::Mat& _distcoeffs) {
    std::vector<cv::Point2f> reference_Img;

    cv::projectPoints(reference_Obj_, _rvec, _tvec, _cameraMatrix, _distcoeffs, reference_Img);
    // 绘制坐标系
  ...
  float getPitch(float _dist, float _tvec_y, float _ballet_speed, const int _company = 1) {
    ...
    for (size_t i = 0; i != 20; ++i) {
      ...
      // 子弹飞行时间
      float t =
          (float((exp(0.001 * _dist) - 1) / (0.001 * _dist * 0.001 * _ballet_speed * cos(a))));
