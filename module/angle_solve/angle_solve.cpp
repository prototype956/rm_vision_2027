...
void solve::angleSolve(cv::RotatedRect object, int row, int col, uart::SerialPort& info) {
  ...
  fmt::print("[{}] Distance: {:.2f} Height: {:.2f} object_y: {:.2f}\n", angle_info,
             target.predict.y, target.predict.z, object.center.y);
  target.time = target.predict.y / ((info.returnReceiveBulletVelocity() * config.speed_arg) *
                                    cos(-info.returnReceivePitch() / 180 * PI) * 100);
  ...
  target.pitch =
      atan(-target.predict.z / sqrt(pow(target.predict.y, 2) + pow(target.predict.x, 2))) / PI *
      180;
